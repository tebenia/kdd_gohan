...
12. **TALLY/LIST = RETURN VALUES ONLY**: When the question says "tally", "list", or "enumerate" X, return only the X values (one per distinct value, one row each). Do NOT add a count/frequency column or linking IDs. "Tally the elements" → column `element`, rows = the distinct element values. Adding a count column = score of 0.
...
19. For toxicology questions asking for the Nth atom of each molecule, parse the numeric suffix of `atom_id` exactly. `LIKE '%_4'` is wrong because it also matches atoms such as `TR001_14` and `TR001_24`; lexicographic row order is also wrong because `TR001_10` sorts before `TR001_2`. Use `atom_id.split('_')[-1] == '4'` in Python, or cast the suffix after the underscore to an integer in SQL, then apply the tally/list distinct-values rule.
20. For Formula 1 race-time percentage questions such as "how much faster in percentage is the champion than the driver who finished the race last", use `results.milliseconds` for the champion and the last driver with a non-null recorded `milliseconds` value. Formula: `(last_ms - champion_ms) * 100 / last_ms`. Return the raw numeric value with full precision; do not round.
21. If a `.md` or `.txt` file is large, or a preview/search did not expose the needed rows, use `execute_python` to read/search the full file. Do not repeat the same tool call with the same parameters after it failed or gave no new information; change strategy.
22. For computed numeric answers such as percentages, ratios, averages, or formula results, return the raw numeric value with full precision. If the question says percentage or uses `% of`, return the percent-scale value after multiplying by 100. If it says ratio, rate, proportion, or fraction, return the raw fraction. Do not add `%`, units, symbols, or rounded display strings unless the question explicitly asks for that formatting.
...
- If an explicit question or schema-specific rule does require `LIMIT 1` for a min/max result, add a stable secondary sort key such as the requested entity/name/id column: `ORDER BY metric_col ASC, entity_name_col ASC LIMIT 1`. Do not rely on the metric column alone when ties are possible.
...
- For conditional percentage questions of the form "What percentage of Y are X?", keep the denominator as all Y rows and use `CASE WHEN` or equivalent logic for X in the numerator. Do not put the X condition in `WHERE`, because that turns the result into 100%.
...
- When a narrative `.md` document stores entity facts by ID, first resolve names to IDs from CSV/JSON tables, then search the document by ID. Collect every paragraph mentioning the same ID before deciding; category, amount, status, and links may be deliberately scattered across separate paragraphs.
...
- In student_club budget-ratio questions asking "how many times was the budget in <category> for event A more than event B", use the allocated budget `amount`, not `spent`, `expense.cost`, row counts, or a difference. Filter the requested budget category, resolve both event names, compute `amount_for_event_A / amount_for_event_B`, and return the raw numeric ratio. When reading `doc/budget.md`, prefer the final revised amount over provisional, spent, or invoiced amounts.
...
- In superhero publisher percentage questions, resolve publisher labels through `publisher_name`, not default or placeholder `publisher_id` values. If JSON fragments look incomplete or repeated, read `doc/superhero.md` and merge attributes by hero id to recover the corrected height and publisher affiliation before computing the denominator and numerator.
...
- For Formula 1 race-time percentage questions, calculate from `results.milliseconds`, not formatted `time`. Use the last finisher row that has non-null `milliseconds`; drivers with null `milliseconds` do not have a comparable total race time. Use `(last_ms - champion_ms) * 100 / last_ms` and return the raw numeric result, not a rounded display value.
...
- In laboratory abnormality questions, use numeric normal-range thresholds from `knowledge.md` or `doc/Laboratory.md`; do not keyword-match descriptions such as high, elevated, or abnormal. Count distinct patient IDs and use patient-level set logic when multiple lab rows exist.
- In toxicology molecule tasks, atom position is the integer suffix in `atom_id` after the underscore, not the lexicographic row number and not a SQL `LIKE '%_N'` suffix pattern. For "tally the element of the 4th atom of each carcinogenic molecule", filter carcinogenic molecules, keep rows where the parsed atom suffix equals 4, then return `SELECT DISTINCT element` only.
- In card legality percentage tasks, if there is no `legalities` table, parse `doc/legalities.md`: map legality entry ids to `cards_id`, filter entries whose format and status text match the requested values such as Commander and Legal, join to `cards.id`, then compute the requested percentage from `cards` columns such as `hasContentWarning`.
