...
STUDENT_CLUB_BUDGET_RATIO_CODE = "student_club_budget_ratio_uses_allocated_amounts"
CARD_LEGALITY_CONTENT_WARNING_PERCENTAGE_CODE = (
    "card_legality_content_warning_percentage_uses_doc_legalities"
)
...
def validate_answer(
    ...
    *,
    ...
    issues.extend(
        _cost_question_used_amount_without_cost_issues(
            task.question,
            columns,
            rows,
            schema_tables,
    ...
    issues.extend(_student_club_budget_ratio_issues(task, rows))
    issues.extend(_card_legality_content_warning_percentage_issues(task, rows))
...
def _cost_question_used_amount_without_cost_issues(
    question: str,
    columns: Sequence[str],
    rows: Sequence[Sequence[Any]],
    schema_tables: Sequence[dict[str, Any]],
    ...
    if not _question_has_minmax(question):
        return []
    if not _COST_METRIC_PATTERN.search(question):
        return []
    if len(columns) != 1 or not rows or _is_helper_column(columns[0]):
        return []
    if not _schema_has_column(schema_tables, "cost") or not _schema_has_column(
        schema_tables,
        "amount",
    ):
        return []
    ...
    if not history_text:
        return []
    if not _AMOUNT_COLUMN_PATTERN.search(history_text):
        return []
    if _COST_METRIC_PATTERN.search(history_text):
        return []

    return [
        AnswerValidationIssue(
            code="cost_minmax_query_used_amount_without_cost",
            message=(
                "The question asks for a cost-based min/max answer, and the context has a "
                "`cost` column, but the recorded query used `amount` without referencing "
                "`cost`. Use the literal `cost` metric for the min/max lookup; only fall "
                "back to `amount` when no `cost` column exists."
            ),
        )
    ]
...
REC_ID_PATTERN = re.compile(r"\brec[A-Za-z0-9]{14}\b")


def _quoted_phrases(question: str) -> list[str]:
    return [match.strip() for match in re.findall(r"[\"']([^\"']+)[\"']", question) if match.strip()]


def _student_club_budget_ratio_request(question: str) -> tuple[str, str, str] | None:
    lowered_question = question.lower()
    if not (
        "budget" in lowered_question
        and "how many times" in lowered_question
        and "more than" in lowered_question
    ):
        return None

    event_names = _quoted_phrases(question)
    if len(event_names) < 2:
        return None

    category_match = re.search(
        r"\bbudget\s+in\s+(?P<category>[A-Za-z][A-Za-z0-9 &/_-]*?)\s+for\b",
        question,
        flags=re.IGNORECASE,
    )
    ...
    if not category:
        return None
    return category, event_names[0], event_names[1]
...
def _event_ids_by_name(context_dir: Path) -> dict[str, str]:
    event_ids: dict[str, str] = {}
    for path in sorted(context_dir.rglob("event.csv")):
        try:
            with path.open(newline="") as handle:
                reader = csv.DictReader(handle)
                if not {"event_id", "event_name"}.issubset(reader.fieldnames or []):
                    continue
                for row in reader:
                    event_name = str(row.get("event_name") or "").strip()
                    event_id = str(row.get("event_id") or "").strip()
                    if event_name and event_id:
                        event_ids[event_name.lower()] = event_id
        except Exception:
            continue
    return event_ids


def _paragraphs_by_record_id(text: str) -> dict[str, list[str]]:
    paragraphs_by_id: dict[str, list[str]] = {}
    for paragraph in re.split(r"\n\s*\n", text):
        ids = set(REC_ID_PATTERN.findall(paragraph))
        for record_id in ids:
            paragraphs_by_id.setdefault(record_id, []).append(paragraph)
    return paragraphs_by_id


def _budget_category_matches(paragraphs: Sequence[str], category: str) -> bool:
    normalized_category = _normalize_free_text(category)
    return any(normalized_category in _normalize_free_text(paragraph) for paragraph in paragraphs)


def _budget_event_ids(paragraphs: Sequence[str], known_event_ids: set[str]) -> set[str]:
    event_ids: set[str] = set()
    for paragraph in paragraphs:
        event_ids.update(record_id for record_id in REC_ID_PATTERN.findall(paragraph) if record_id in known_event_ids)
    return event_ids


def _budget_amount_candidates(paragraph: str) -> list[tuple[int, float]]:
    candidates: list[tuple[int, float]] = []
    sentences = re.split(r"(?<=[.!?])\s+", paragraph)
    for sentence in sentences:
        lowered_sentence = sentence.lower()
        if not re.search(
            r"\b(amount|allocated|allocation|funded|budgeted|budget|revised|adjusted|"
            r"confirmed|approved)\b",
            lowered_sentence,
        ):
            continue
        if not re.search(
            r"\b(amount|allocated|allocation|funded|budgeted|budget|revised|adjusted)\b",
            lowered_sentence,
        ):
            continue
        ...
        priority = (
            2
            if re.search(
                r"\b(final|revised|corrected|confirmed|approved|reconciled|adjusted)\b",
                lowered_sentence,
            )
            else 1
        )
        for match in re.finditer(r"(?<![A-Za-z])\d+(?:\.\d+)?", sentence):
            nearby_text = lowered_sentence[max(0, match.start() - 36) : match.end() + 24]
            if re.search(r"\b(spent|remaining|balance|expenditure|expenditures|costs?|invoiced)\b", nearby_text):
                continue
            try:
                candidates.append((priority, float(match.group(0))))
            except ValueError:
                continue
    return candidates


def _budget_amount_from_paragraphs(paragraphs: Sequence[str]) -> float | None:
    candidates: list[tuple[int, float]] = []
    for paragraph in paragraphs:
        candidates.extend(_budget_amount_candidates(paragraph))
    if not candidates:
        return None
    best_priority = max(priority for priority, _ in candidates)
    priority_values = [value for priority, value in candidates if priority == best_priority]
    return priority_values[-1]


def _student_club_budget_ratio_column(numerator_event: str, denominator_event: str) -> str:
    numerator = numerator_event.replace("'", "''")
    denominator = denominator_event.replace("'", "''")
    return (
        f"CAST(SUM(CASE WHEN T2.event_name = '{numerator}' THEN T1.amount ELSE 0 END) "
        f"AS REAL) / SUM(CASE WHEN T2.event_name = '{denominator}' THEN T1.amount ELSE 0 END)"
    )


def expected_student_club_budget_ratio_answer(task: PublicTask) -> tuple[str, float] | None:
    request = _student_club_budget_ratio_request(task.question)
    if request is None:
        return None
    category, numerator_event, denominator_event = request

    event_ids_by_name = _event_ids_by_name(task.context_dir)
    numerator_event_id = event_ids_by_name.get(numerator_event.lower())
    denominator_event_id = event_ids_by_name.get(denominator_event.lower())
    if numerator_event_id is None or denominator_event_id is None:
        return None

    budget_doc_path = _find_context_file(task.context_dir, "budget.md")
    if budget_doc_path is None:
        return None

    try:
        budget_text = budget_doc_path.read_text(errors="replace")
    except Exception:
        return None

    known_event_ids = set(event_ids_by_name.values())
    paragraphs_by_id = _paragraphs_by_record_id(budget_text)
    amounts_by_event = {numerator_event_id: 0.0, denominator_event_id: 0.0}
    found_events: set[str] = set()
    for paragraphs in paragraphs_by_id.values():
        if not _budget_category_matches(paragraphs, category):
            continue
        amount = _budget_amount_from_paragraphs(paragraphs)
        if amount is None:
            continue
        for event_id in _budget_event_ids(paragraphs, known_event_ids):
            if event_id not in amounts_by_event:
                continue
            amounts_by_event[event_id] += amount
            found_events.add(event_id)

    if not {numerator_event_id, denominator_event_id}.issubset(found_events):
        return None
    denominator_amount = amounts_by_event[denominator_event_id]
    if denominator_amount == 0:
        return None

    column = _student_club_budget_ratio_column(numerator_event, denominator_event)
    return column, amounts_by_event[numerator_event_id] / denominator_amount


def _student_club_budget_ratio_issues(
    task: PublicTask,
    rows: Sequence[Sequence[Any]],
) -> list[AnswerValidationIssue]:
    ...
    observed = _single_numeric_answer(rows)
    if observed is not None and abs(observed - expected_value) <= max(1.0, abs(expected_value)) * 1e-12:
        return []

    return [
        AnswerValidationIssue(
            code=STUDENT_CLUB_BUDGET_RATIO_CODE,
            message=(
                "For this student-club budget-ratio question, collect all `budget.md` paragraphs "
                "for each budget id, use the allocated/final `amount`, link the budget id to the "
                "named events, and compute numerator-event amount divided by denominator-event "
                f"amount. The context-derived raw ratio is {expected_value}."
            ),
        )
    ]


def _card_legality_percentage_request(question: str) -> tuple[str, str, bool] | None:
    lowered_question = question.lower()
    if not (
        "percentage" in lowered_question
        and "card" in lowered_question
        and "format" in lowered_question
        and "legal status" in lowered_question
        and "content warning" in lowered_question
    ):
        return None
    if not re.search(r"\b(do not|don't|without|no)\s+(?:have\s+)?(?:a\s+)?content warning\b", lowered_question):
        return None

    format_match = re.search(
        r"\bformat\s+(?P<format>[a-z0-9 _-]+?)\s+and\s+legal\s+status\b",
        lowered_question,
    )
    if format_match is None:
        return None
    card_format = format_match.group("format").strip()
    if not card_format:
        return None
    return _normalize_free_text(card_format), "legal", False


def _legality_id_from_paragraph(paragraph: str) -> int | None:
    match = re.search(r"\bID\s+(?P<id>\d+)\b", paragraph, flags=re.IGNORECASE)
    if match is None:
        return None
    try:
...
def _legality_doc_maps(context_dir: Path) -> tuple[dict[int, int], dict[int, str], dict[int, str]] | None:
    legalities_doc_path = _find_context_file(context_dir, "legalities.md")
    if legalities_doc_path is None:
        return None

    try:
        legalities_text = legalities_doc_path.read_text(errors="replace")
    except Exception:
        return None

    cards_by_legality_id: dict[int, int] = {}
    formats_by_legality_id: dict[int, str] = {}
    statuses_by_legality_id: dict[int, str] = {}
    for paragraph in re.split(r"\n\s*\n", legalities_text):
        legality_id = _legality_id_from_paragraph(paragraph)
        if legality_id is None:
            continue

        card_ids = re.findall(r"\bcards_id\s+(\d+)\b", paragraph, flags=re.IGNORECASE)
        if card_ids:
            try:
                cards_by_legality_id[legality_id] = int(card_ids[-1])
            except ValueError:
                pass

        normalized_paragraph = _normalize_free_text(paragraph)
        if re.search(r"\bcommander\b", normalized_paragraph):
            formats_by_legality_id[legality_id] = "commander"
        if re.search(r"\blegal\b", normalized_paragraph):
            statuses_by_legality_id[legality_id] = "legal"

    return cards_by_legality_id, formats_by_legality_id, statuses_by_legality_id


def _card_content_warning_values(context_dir: Path, card_ids: set[int]) -> dict[int, int]:
    if not card_ids:
        return {}

    cards_db_path = _find_context_file(context_dir, "cards.db")
    ...
    try:
        with sqlite3.connect(cards_db_path) as conn:
            rows = conn.execute(
                f"SELECT id, hasContentWarning FROM cards WHERE id IN ({placeholders})",
    ...
    values: dict[int, int] = {}
    for card_id, has_content_warning in rows:
        try:
            values[int(card_id)] = int(has_content_warning)
        except (TypeError, ValueError):
            continue
    return values


def expected_card_legality_content_warning_percentage_answer(
    task: PublicTask,
) -> tuple[str, float] | None:
    request = _card_legality_percentage_request(task.question)
    if request is None:
        return None
    requested_format, requested_status, wants_warning = request

    doc_maps = _legality_doc_maps(task.context_dir)
    if doc_maps is None:
        return None
    cards_by_legality_id, formats_by_legality_id, statuses_by_legality_id = doc_maps

    matching_card_ids = {
        card_id
        for legality_id, card_id in cards_by_legality_id.items()
        if formats_by_legality_id.get(legality_id) == requested_format
        and statuses_by_legality_id.get(legality_id) == requested_status
    }
    warning_values = _card_content_warning_values(task.context_dir, matching_card_ids)
    if not warning_values:
        return None

    denominator = len(warning_values)
    if wants_warning:
        numerator = sum(1 for value in warning_values.values() if value != 0)
    else:
        numerator = sum(1 for value in warning_values.values() if value == 0)

    column = "CAST(SUM(CASE WHEN T1.hasContentWarning = 0 THEN 1 ELSE 0 END) AS REAL) * 100 / COUNT(T1.id)"
    return column, numerator * 100.0 / denominator


def _card_legality_content_warning_percentage_issues(
    task: PublicTask,
    rows: Sequence[Sequence[Any]],
) -> list[AnswerValidationIssue]:
    ...
    return [
        AnswerValidationIssue(
            code=CARD_LEGALITY_CONTENT_WARNING_PERCENTAGE_CODE,
            message=(
                "For this card legality percentage question, parse `doc/legalities.md` by "
                "legality ID, map each legality ID to its final `cards_id`, filter legality "
                "entries by the requested format and Legal status, join to `cards.id`, then "
                "compute the content-warning percentage with the legal-format cards as the "
                f"denominator. The context-derived raw percentage is {expected_value}."
            ),
        )
    ]
