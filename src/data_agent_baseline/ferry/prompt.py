...
10. You have `duckdb` (v1.1.0 or later) installed. Use it for complex joins across different file formats. CRITICAL: When `knowledge.md` shows a SQL example but the data is in CSV/JSON format (no .db file), translate that SQL to DuckDB using registered DataFrames — do NOT use pandas sort. CRITICAL NULL ORDERING: In DuckDB, `ORDER BY col ASC` puts NULLs LAST, but in SQLite `ORDER BY col ASC` puts NULLs FIRST. When using DuckDB (including via `execute_context_duckdb`) for any query with `ORDER BY col ASC LIMIT 1`, ALWAYS append `NULLS FIRST` to match SQLite NULL behavior: `ORDER BY col ASC NULLS FIRST LIMIT 1`. This applies regardless of whether data is in CSV/JSON or .db format. The first row returned IS the final answer — do NOT run a second query to find the non-NULL minimum and override it. Example pattern for CSV+JSON data:
```python
...
- EXCEPTION — when a schema-specific rule or explicit instruction requires `LIMIT 1` for a min/max question, ALWAYS use two sort keys for deterministic tie-breaking: `ORDER BY metric_col ASC, entity_name_col ASC LIMIT 1`. Never rely on a single sort key (`ORDER BY metric_col ASC LIMIT 1`) — tied metric values give a non-deterministic (arbitrary) row in DuckDB. The secondary `entity_name_col ASC` ensures the alphabetically-first entity is returned consistently.
...
```json
{"thought":"Self-check FAILED: I have 12 rows but question asks for lowest cost event. I used SUM/GROUP BY — should use ORDER BY ex.cost ASC, e.event_name ASC LIMIT 1 (two sort keys for deterministic tie-breaking). Re-executing.","actions":[{"action":"execute_context_sql","action_input":{"path":"data.db","sql":"SELECT e.event_name FROM expense ex JOIN budget b ON ex.link_to_budget=b.budget_id JOIN event e ON b.link_to_event=e.event_id ORDER BY ex.cost ASC, e.event_name ASC LIMIT 1"}}]}
```
...
- Bad pattern 2: ORDER BY ex.cost ASC LIMIT 1 — single sort key is non-deterministic when multiple rows tie at the same cost value
- Good pattern: ORDER BY ex.cost ASC, e.event_name ASC LIMIT 1 — TWO sort keys: primary metric ASC, then entity name ASC for deterministic tie-breaking
